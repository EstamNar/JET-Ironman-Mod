...
COMPATIBILITY: This mod should be compatible with most mods currently released(I haven't tested them all, lol). Kovacs Altered Escape 2.1.0 is confirmed compatible, as well as EFT: Rebirth, and either recommended for an even more brutal, single-player-focused experience. Any future mod that modifies any of the same files or values as my mod, or tampers with the offraid.js, may have compatibility issues. If you find any mods that don't play nice with mine, let me know on any of the Emu discords, or DM me directly. 

INSTALL: Extract folder "EstamNar-IronmanMod-1.0.5" into your (server)/user/mods folder. Delete the "cache" folder in (server)/user, and restart the server. 

UNINSTALL: Delete folder "EstamNar-IronmanMod-1.0.5" from your (server)/user/mods folder. Delete the "cache" folder in (server)/user, and restart the server. 

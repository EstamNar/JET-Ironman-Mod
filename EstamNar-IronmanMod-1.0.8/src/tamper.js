exports.mod = (mod_info) => {
    logger.logInfo("[MOD] Estams Ironman Mod");

    let userconfig = fileIO.readParsed(internal.path.resolve(__dirname, "../USER_CONFIG.json"))

    //Softwipe Mod
    if (userconfig.softWipeOnDeath) {

        if (server.version == "1.1.1" || server.version == "1.2.0") offraid_f.saveProgress = require("./offraidchanges1.1.1").saveProgress;
        else
            if (server.version == "0.12.4-1012-Dev") offraid_f.saveProgress = require("./offraidchanges1.1.2").saveProgress
            else
                if (server.ServerVersion == "1.2.0 v12.12-dev") offraid_f.saveProgress = require("./offraidchangesAE12.12").saveProgress;
                else offraid_f.saveProgress = require("./offraidchanges1.1.0").saveProgress;
    }

    //Tweaks Mod

    //Compatability for traditional server versions
    if (server.version == "1.1.1" || server.version == "1.2.0" || server.version == "0.12.4-1012-Dev") {
        ...
        if (server.version == "1.2.0" || server.version == "0.12.4-1012-Dev") {
    ...
    //Compatability for AE 12.12's lack of a working server version... >:|
    else {
        let tweaks = _database.globals.config;
        ...
        tweaks.RagFair.enabled = !userconfig.disableRagfair;
        _database.globals.config = tweaks
    }
    ...
    if (userconfig.smallStashes) {
        let smallItems = _database.items;
        ...
        for (index in stashIds) {
            ...
            smallItems[standard]._props.Grids[0]._props.cellsV = 4;
            smallItems[behind]._props.Grids[0]._props.cellsV = 8;
            smallItems[prepare]._props.Grids[0]._props.cellsV = 12;
            smallItems[eod]._props.Grids[0]._props.cellsV = 16;
        }
        _database.items = smallItems;
    }
    ...
    if (server.version == "1.1.1" || server.version == "1.2.0" || server.version == "0.12.4-1012-Dev") {
        ...
        if (userconfig.disableTraders) {
            let keys = Object.keys(_database.traders);
            let TraderCache = internal.path.resolve(__dirname, "categories.json");
            for (index in keys) {
                let id = keys[index];
                if (id.toLowerCase() == "ragfair") continue;
                let dedTraders = _database.traders[id].base;
                let IronmanTraders = internal.path.resolve(__dirname, id + "base.json");

                dedTraders.sell_category = [];
                _database.traders[id].base = IronmanTraders;
                _database.traders[id].categories = TraderCache;
            }
            fileIO.write(TraderCache, []);
            keys = Object.keys(db.user.cache).filter(x => x.includes("assort_"));
            for (index in keys) {
    ...
    if (!userconfig.disableNightmares) {
        let Locations = _database.locations;
    ...
    logger.logSuccess("[MOD] Estams Ironman Mod; Applied");
}